...
__all__ = ["SPADEDiffusionModelUNet", "ConditionDiffusionModelUNet"]
...
class SPADEDiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        for upsample_block in self.up_blocks:
            ...
            print(f'Up block output shape: {h.shape}')
...
class ConditionDiffusionModelUNet(nn.Module):
    def __init__(
        self,
        spatial_dims: int,
        in_channels: int,
        out_channels: int,
        label_nc: Sequence[int] | int,
        num_res_blocks: Sequence[int] | int = (2, 2, 2, 2),
        channels: Sequence[int] = (32, 64, 64, 64),
        attention_levels: Sequence[bool] = (False, False, True, True),
        norm_num_groups: int = 32,
        ...
        # TODO
        if isinstance(label_nc, (list, tuple)):
            # 如果是列表或元组，检查长度
            if len(label_nc) > 0 and len(label_nc) != len(channels):
                raise ValueError("label_nc should have same length as channels or only one value.")
        elif isinstance(label_nc, int):
            # 如果是整数，长度检查不适用
            self.label_nc = (label_nc,) * len(channels)
        else:
            raise TypeError(f"label_nc should be int, list or tuple, got {type(label_nc)}")
    ...
    def forward(
        ...
        Args:
            x: input tensor (N, C, SpatialDims).
            timesteps: timestep tensor (N,).
            TODO seg: Bx[LABEL_NC]x[SPATIAL DIMENSIONS] tensor of condition_map for SPADE norm.
            context: context tensor (N, 1, ContextDim).
            class_labels: context tensor (N, ).
            down_block_additional_residuals: additional residual tensors for down blocks (N, C, FeatureMapsDims).
            mid_block_additional_residual: additional residual tensor for mid block (N, C, FeatureMapsDims).
        ...
        for downsample_block in self.down_blocks:
            ...
            for residual in res_samples:
                down_block_res_samples.append(residual)
            # print(f"encoder block output shape: {h.shape}; res_samples shapes: {[r.shape for r in res_samples]}")
        ...
        if isinstance(seg, torch.Tensor):
            for upsample_block in self.up_blocks:
                ...
                down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
                h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, seg=seg, temb=emb, context=context)
                # print(f"decoder block input shape: {h.shape}; res_samples shapes: {[r.shape for r in res_samples]}; ")
        elif isinstance(seg, list):
            condition_map = seg
            for i, upsample_block in enumerate(self.up_blocks):
                res_samples = down_block_res_samples[-len(upsample_block.resnets) :]
                down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
                print(f'Using condition_map with shape: {condition_map[-(i+1)].shape} for upsample block {i}; h input shape: {h.shape}')
                h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, seg=condition_map[-(i+1)], temb=emb, context=context)
                # print(f"decoder block input shape: {h.shape}; res_samples shapes: {[r.shape for r in res_samples]}; condition_map shape: {condition_map[-(i+1)].shape}")
...
if __name__ == "__main__":
    ...
    # seg = torch.randn(1, 64, 56, 56).cuda()
    seg = [torch.randn(1, 64, 56, 56).cuda(), torch.randn(1, 64, 28, 28).cuda(),
           torch.randn(1, 128, 14, 14).cuda(), torch.randn(1, 256, 7, 7).cuda()]
    #
    out = model(x, t, seg)
    print(out.shape)  # Expected output shape: torch.Size([1, 8, 56, 56])
