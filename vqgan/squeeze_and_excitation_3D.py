...
class AxialAttention(nn.Module):
    def __init__(self, num_channels, reduction_ratio=2):
        ...
        num_channels_reduced = num_channels * 2 // reduction_ratio
        self.reduction_ratio = reduction_ratio
        self.relu = nn.ReLU()
        self.conv_c = nn.Conv3d(in_channels=num_channels * 2, out_channels=num_channels_reduced, kernel_size=1, stride=1)
        self.conv_cT = nn.Conv3d(in_channels=num_channels_reduced, out_channels=num_channels, kernel_size=1, stride=1)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input_tensor):
        ...
        # Project:
        # Average along channels and different axes
        squeeze_tensor_w1 = F.adaptive_avg_pool3d(input_tensor, (1, 1, W))
        squeeze_tensor_w2 = F.adaptive_max_pool3d(input_tensor, (1, 1, W))
        squeeze_tensor_h1 = F.adaptive_avg_pool3d(input_tensor, (1, H, 1))
        squeeze_tensor_h2 = F.adaptive_max_pool3d(input_tensor, (1, H, 1))
        squeeze_tensor_d1 = F.adaptive_avg_pool3d(input_tensor, (D, 1, 1))
        squeeze_tensor_d2 = F.adaptive_max_pool3d(input_tensor, (D, 1, 1))

        # tile tensors to original siz
        ...
        final_squeeze_tensor2 = sum([squeeze_tensor_w2.view(batch_size, num_channels, 1, 1, W),
                                    squeeze_tensor_h2.view(batch_size, num_channels, 1, H, 1),
                                    squeeze_tensor_d2.view(batch_size, num_channels, D, 1, 1)])

        final_squeeze_tensor = torch.cat([final_squeeze_tensor1, final_squeeze_tensor2], dim=1)
